...
main (void)
{
  ...
  while(1)
  {
      Imprimir_banco(cajero, filas, tiempos, cont, tiempomod, atendidos, aux);  //pendiente
      ...
      if((tiempomod % tiempos[0]) == 0) //Modulo del tiempo con el que son atendidos es 0 hace push de los cajeros.
      {
          ...
          //Bloque preferentes
          while( (Empty(&filas[2]) == FALSE) && (ClyPr < 5) && (cont2 < numero) )
          ...
          //Bloque clientes
          while( (Empty(&filas[0]) == FALSE) && (ClyPr < 6) && (cont2 <= numero) )
